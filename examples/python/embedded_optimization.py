...
- Runtime configuration (OpenVINO, TensorRT, ONNX Runtime, SNPE)
...
def configure_runtime(model, backend: str):
    ...
    if backend == "openvino":
        ...
        print("Configuring SNPE (Qualcomm hardware)...")
        model.configure_snpe(
            runtime="dsp",                   # cpu, gpu, dsp
            performance_profile="HIGH_PERFORMANCE",
        )
        print("  runtime: dsp")
        print("  performance_profile: HIGH_PERFORMANCE")
...
def main():
    ...
    parser.add_argument(
        "--backend",
        type=str,
        choices=["openvino", "tensorrt", "onnxruntime", "snpe"],
        default="onnxruntime",
    ...
    print("   - Qualcomm: SNPE with DSP runtime")
