...
Usage:
    python detection_demo.py --model yolov8n.onnx --device cuda:0 --image bus.jpg
"""
...
def main():
    ...
    # Print SDK info
    print(f"iVIT-SDK Version: {ivit.version()}")
    ...
    detector = ivit.Detector(args.model, device)
    ...
    if args.benchmark:
        ...
        # Warmup
        for _ in range(10):
            detector(image, args.conf, args.iou)

        # Benchmark
        for i in range(args.iterations):
            results = detector(image, args.conf, args.iou)
